...
resource "aws_instance" "cassandra_client" {
  count = local.c_node_count
  ...
  user_data = "${file("configure_ubuntu_cassandra.sh")}"

  tags = {
    Name = "felixm-cassandra-client${count.index}"
    Creator = "felixm"
    Method = "Terraform"
  }
}

resource "aws_instance" "cassandra_tester" {
  ami           = data.aws_ami.ubuntu.id
